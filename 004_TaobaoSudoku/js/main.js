...
((function() { //闭包
	...
	// 隐藏放大遮罩与放大的图片容器 监听事件
	 ...
	 // 放大遮罩移动放大图片 mousemove
	 leftView.addEventListener('mousemove', function(evt){
	 	evt = evt || window.event //e存在就用e不存在就用windon.event
	 	// 获取当前鼠标的位置
	 	let currentMouseX = evt.pageX
	 	let currentMouseY = evt.pageY
	 	// 获取要放大图片左侧与顶部的偏移距离
	 	let offsetLeft = container.offsetLeft //left宽度
	 	let offsetTop = container.offsetHeight //top高度
	 	// 获取放大遮罩的大小
